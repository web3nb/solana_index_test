...
#[substreams::handlers::map]
fn graph_instructions_out(instructions: Instructions) -> Result<EntityChanges, substreams::errors::Error> {
    let mut tables = Tables::new();

    for instruction in instructions.instructions {
        tables
            .create_row("Instruction", instruction.program_id.clone())
            .set("program_id", instruction.program_id)
            .set("accounts", serde_json::to_string(&instruction.accounts).unwrap_or_default())
            .set("data", instruction.data);
    }
